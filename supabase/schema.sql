...
-- Helper functions to break infinite RLS loops (Security Definer bypasses RLS)
...
as $$
begin
  return exists (select 1 from workspace_members where workspace_id = ws_id and user_id = auth.uid() and role in ('owner', 'admin'));
end;
$$;
...
as $$
begin
  return exists (select 1 from workspace_members where workspace_id = ws_id and user_id = auth.uid());
end;
$$;

create policy "Users can view own workspaces" on public.workspaces for
select using (auth.uid() = owner_id OR public.is_workspace_member(id));
...
update using (auth.uid() = owner_id OR public.is_workspace_admin(id));
...
select using (user_id = auth.uid() OR public.is_workspace_owner(workspace_id) OR public.is_workspace_member(workspace_id));

create policy "Users can modify workspace members" on public.workspace_members for
all using (public.is_workspace_owner(workspace_id) OR public.is_workspace_admin(workspace_id));

create policy "Users can view own workspace invites" on public.workspace_invites for
select using (email = auth.jwt()->>'email' OR public.is_workspace_owner(workspace_id) OR public.is_workspace_member(workspace_id));
