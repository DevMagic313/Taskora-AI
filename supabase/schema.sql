...
IF EXISTS (
    ...
    ALTER TABLE public.tasks ADD COLUMN IF NOT EXISTS pending_reason text;
...
IF EXISTS (
    ...
    ALTER TABLE public.task_logs ADD COLUMN IF NOT EXISTS created_at timestamptz default now();
END IF;
...
create policy "Users can view own workspace members" on public.workspace_members
    for select using (
        user_id = auth.uid()
...
create policy "Users can modify workspace invites" on public.workspace_invites
    for all using (
        public.is_workspace_owner(workspace_id)
        OR public.is_workspace_admin(workspace_id)
    );
...
create policy "Users can view own logs" on public.task_logs
    for select using (auth.uid() = user_id);

create policy "Users can insert own logs" on public.task_logs
    for insert with check (auth.uid() = user_id);
...
create policy "Users can view own ai history" on public.ai_generation_history
    for select using (auth.uid() = user_id);
...
create policy "Public Access" on storage.objects
    for select using (bucket_id in ('avatars', 'workspace-logos'));
