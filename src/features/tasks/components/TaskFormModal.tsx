...
export function TaskFormModal({ isOpen, onClose, onSubmit }: TaskFormModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
            ...
            <div className="relative w-full sm:max-w-[500px] rounded-t-[2rem] sm:rounded-[2rem] border border-border/60 bg-card p-1 shadow-2xl animate-scale-in overflow-hidden max-h-[90vh] sm:max-h-[85vh] flex flex-col" onClick={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="modal-title">
                <div className="bg-background/95 backdrop-blur-2xl rounded-t-[1.85rem] sm:rounded-[1.85rem] p-5 sm:p-6 md:p-8 relative z-10 overflow-y-auto flex-1 custom-scrollbar">
                    <div className="mb-6 sm:mb-8 flex items-start justify-between">
                        <div className="flex items-center gap-3 sm:gap-4">
                            ...
                            <div>
                                <h2 id="modal-title" className="text-xl sm:text-2xl font-black tracking-tight">New Action Item</h2>
                                <p className="text-sm font-medium text-muted-foreground mt-0.5">Define your next objective.</p>
                            </div>
                        </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                            <div className="space-y-2 relative group">
                                <label htmlFor="task-start-date" className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80 flex items-center gap-2 mb-2 ml-1">
                                    Start Date
                                </label>
                            ...
                            <div className="space-y-2 relative group">
                                <label htmlFor="task-due-date" className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80 flex items-center gap-2 mb-2 ml-1">
                                    Deadline
                                </label>
                        ...
                        <div className="space-y-2 relative group">
                            <label htmlFor="task-comments" className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80 gap-2 mb-2 ml-1 block">
                                Comments
                            </label>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 sm:gap-6">
                            <div className="space-y-2 relative group">
                                <label htmlFor="task-notes" className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80 block mb-2 ml-1">
                                    Notes
                                </label>
                                <textarea id="task-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Personal notes..." className="flex min-h-[60px] w-full rounded-xl sm:rounded-2xl border-2 border-input bg-card/50 px-4 py-3 text-sm font-medium transition-all duration-300 hover:border-border focus-visible:outline-none focus-visible:ring-0 focus-visible:border-primary disabled:opacity-50 resize-vertical shadow-sm" disabled={isSubmitting} />
                            </div>
                            <div className="space-y-2 relative group">
                                <label htmlFor="task-remarks" className="text-xs font-bold uppercase tracking-wider text-muted-foreground/80 block mb-2 ml-1">
                                    Remarks
                                </label>
                                <textarea id="task-remarks" value={remarks} onChange={(e) => setRemarks(e.target.value)} placeholder="Extra remarks..." className="flex min-h-[60px] w-full rounded-xl sm:rounded-2xl border-2 border-input bg-card/50 px-4 py-3 text-sm font-medium transition-all duration-300 hover:border-border focus-visible:outline-none focus-visible:ring-0 focus-visible:border-primary disabled:opacity-50 resize-vertical shadow-sm" disabled={isSubmitting} />
                            </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-4 sm:pt-6 border-t border-border/40 mt-6 sm:mt-8">
                            ...
                            <div className="flex gap-3 ml-auto">
                                ...
                                <Button id="task-submit-btn" type="submit" size="lg" isLoading={isSubmitting} disabled={!title.trim()} className="px-6 sm:px-8 shadow-xl shadow-primary/20 hover:shadow-primary/30">
